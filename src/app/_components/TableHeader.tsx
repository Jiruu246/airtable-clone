...
import { MdOutlineTextFields, MdOutline123, MdOutlineKeyboardBackspace } from "react-icons/md";
...
export function TableHeader({ columns, virtualColumns, totalWidth, onAddColumn, isAddingColumn }: TableHeaderProps) {
  ...
  const handleCreateField = () => {
    if (onAddColumn && fieldName.trim() && selectedType) {
      onAddColumn(selectedType, fieldName.trim());
      handleClose();
    }
  };
              ...
              {!selectedType ? (
                <>
                  <div className="px-4 py-3 border-b border-gray-200">
                    <h3 className="text-sm font-medium text-gray-900">Select field type</h3>
                  </div>
                  <DropdownItem
                    icon={<MdTextFormat className="w-4 h-4" />}
                    label="Single line text"
                    onClick={() => setSelectedType('TXT')}
                  />
                  <DropdownItem
                ...
                <>
                  ...
                  <div className="px-4 pb-4">
                    ...
                    >
                      {selectedType === 'TXT' ? (
                        <div className="flex gap-2 items-center">
                          <MdTextFormat className="w-4 h-4 text-gray-500" />
                          <p>Single line text</p>
                        </div>
                      ) : (
                        <div className="flex gap-2 items-center">
                          <AiOutlineNumber className="w-4 h-4 text-gray-500" />
                          <p>Number</p>
                        </div>
                      )}
                    ...
                    { selectedType === 'TXT' ? (
                      <p className="mt-2 text-xs text-gray-700">Enter text, or prefill each new cell with a default value.</p>
                    ) : (
                      <p className="mt-2 text-xs text-gray-700">Enter a number, or prefill each new cell with a default value.</p>
                    )}
                  </div>
                  <div className="px-4 pb-4">
